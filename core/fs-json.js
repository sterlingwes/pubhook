var Promise = require('es6-promise').Promise
  ...
  , _ = require('lodash')
  , slug = require('uslug')
; // chg from md, removed marked & yaml requires
...
var listFiles = function(m, listed) {
  ...
  var source = './' + (m.source ? m.source.replace(/^[\/\.]+/,'') : 'json'); // chg from md
  glob(source + '/**/*.json', function(err,files) { // chg from md
 ...
 * @return {Array} of promises for files to read & process
 */
...
var adapter = function(m) {
  ...
  return new Promise(function(yes,no) {
    ...
    listFiles(m, function(err,files) {
      ...
      Promise.all(readFiles(files.map(function(f) { return f.replace(/^\.+/,cwd); })))
        .then(function(res) {
          ...
          var fnames = _.map(_.pluck(res,'_id'), function(fn, i) {
              var shrunk = slug(fn.replace(cwd+'/'+(m.source ? m.source.replace(/^[\/\.]+/,'') : 'json') + '/','').replace(/\.json$/,''), {allowedChars: '_-/'}); // chg from md
              res[i]._id = shrunk;
              return shrunk;
            })
            , hasChildren = {}
